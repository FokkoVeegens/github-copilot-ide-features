...
def _parse_tag_version(tag: str) -> str:
    ...
    if not match:
        raise ValueError(f"Unrecognised Xcode tag: {tag!r}")
...
def fetch(ide_config: dict) -> list[dict]:
    ...
    for item in raw_releases:
        ...
        try:
            ...
            print(f"  [warn] Skipping Xcode release with unexpected tag: {tag_name!r}")
            continue
        ...
        results.append(
            {
                ...
                "title": item.get("name") or f"GitHub Copilot for Xcode {version}",
